...
namespace IBM.Watsson.Examples
{
    public class ExampleStreaming : MonoBehaviour
    {
        ...
        private ExampleAssistantV2 sn;

        void Start()
        {
            LogSystem.InstallDefaultReactors();
            GameObject go = GameObject.Find("BlueSuitFree01");
            sn = go.GetComponent<ExampleAssistantV2>();
        ...
        public bool Active
        {
            ...
            set
            {
                if (value && !_service.IsListening)
                {
                    ...
                    _service.SilenceThreshold = 0.001f;
        ...
        private void OnRecognize(SpeechRecognitionEvent result)
        {
            ...
            if (result != null && result.results.Length > 0)
            {
                ...
                if(sendMessage)
                {
                    VirtualSceneManager.GetVirtualSceneManager().CallAddPlayerAchievement("InteractWithUsers"); // TODO: example
                    Runnable.Run(sn.TextToSpeechAction(sendText));
                }
            }
        }
